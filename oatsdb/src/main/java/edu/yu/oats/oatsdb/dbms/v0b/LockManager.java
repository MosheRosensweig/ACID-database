...
public enum LockManager
{
     ...
     * Check#1, sleep#1, Check#2, sleep#2, Check#3, break
     *
     * @param tableName
     * @param key
     * @param <K>
    ...
    <K> boolean getResourceLock(String tableName, K key) throws InterruptedException, SystemException
    {
        ...
        if (verbose)
            logger.debug("The default iJustGotTheLock should be 'false', actual value : " + iJustGotTheLock.get());
        iJustGotTheLock.set(false);
        ...
        while (!gotLock && (numberOfAttempts++ < NUMBER_OF_ATTEMPTS)) { //keep trying to get the lock until I get it, or I pass the number of allowed attempts
            ...
            if (lockingThread.equals(currThread)) {                     //if I successfully locked it, exit
                gotLock = true;
                if (leffTestLog)
                    logger.debug("TESTING - I successfully got the lock for table = " + tableName + ", key = " + key);
            } else {                                                    //if someone else has the lock
                if (leffTestLog || tripleTryCheck)
                    logger.debug("TESTING - I did not get the lock. This was attempt " + numberOfAttempts +
                            "/2, for table = " + tableName + ", key = " + key);
                addToWaitList(lockingThread, currThread);               //tell the locking thread to wake me up when he's done
                if (leffTestLog)
                    logger.debug("TESTING - sleeping for " + DBMSImpl.Instance.getTxTimeoutInMillis() + ", ms");
                Thread.sleep(DBMSImpl.Instance.getTxTimeoutInMillis());      //avoid the 0 problem
                if (leffTestLog) logger.debug("TESTING - I woke up!");
            }
        }
        ...
        if (leffTestLog)
            logger.debug("TESTING - I finished getting the lock for table = " + tableName + ", key = " + key);
    ...
    synchronized private void wakeUpWaitingList(Thread lockThread)
    {
        ...
        if (waitingLists.containsKey(lockThread)) {                      //if this thread has a waitinglist
            ...
            for (Thread asleep : waitingList) {
                //synchronized (asleep) {
                //asleep.notify();           //wake up all waiting threads
                asleep.interrupt();
                //}
            }
        }
